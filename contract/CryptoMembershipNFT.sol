...
contract CryptoMembershipNFT is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    {
        // ตรวจสอบว่าผู้ส่งคำขอเป็น owner หรือไม่
        bool isOwnerRegistering = msg.sender == owner();

        // ถ้าผู้ส่งคำขอไม่ใช่ owner ให้ตรวจสอบว่าสมัครแพลน 1 เท่านั้น
        if (
        ...
        // ตรวจสอบว่าแพลนที่เลือกต้องอยู่ในช่วงที่มีและต้องเปิดใช้งานอยู่
        if (_planId == 0 || _planId > state.planCount) {
            revert ContractErrors.InvalidPlanID();
        }
        ...
        if (!state.firstMemberRegistered) {
            ...
            if (_upline == owner()) {
                ...
                // เพิ่มเงื่อนไข: ข้ามการตรวจสอบถ้าผู้สมัครเป็น owner
        ...
        noReentrantTransfer
    {
        ...
        // ตรวจสอบว่า planId ถูกต้อง
        if (_newPlanId == 0 || _newPlanId > state.planCount) {
            revert ContractErrors.InvalidPlanID();
        }

        // ตรวจสอบว่าแพลนเปิดใช้งานอยู่หรือไม่
        ...
        if (!isOwnerUpgrading) {
            ...
            // สำหรับ owner ต้องตรวจสอบว่า newPlanId > planId ปัจจุบัน
            if (_newPlanId <= member.planId) {
                revert ContractErrors.InvalidPlanID();
            }
        }

        // คำนวณส่วนต่างของราคา
        uint256 priceDifference = plans[_newPlanId].price -
            plans[oldPlanId].price;

        // เรียกฟังก์ชันที่แยกออกมาเพื่อดำเนินการส่วนที่เหลือ
